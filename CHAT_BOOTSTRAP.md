...
Huidige stand:
- Laatste gepubliceerde release is `v3.1.79`.
- Public updater assets staan in `pcvantol/djconnect-pi-releases`, inclusief `djconnect-pi-latest.json`.
- Release `v3.1.79` is gepubliceerd in source en public distribution repo.
- Public assets voor `v3.1.79` zijn geverifieerd:
  - `djconnect-pi-3.1.79.tar.gz`
  - `djconnect-pi-3.1.79.sha256`
  - `djconnect-pi-latest.json`
- Cleanup is gedraaid met `./cleanup_old_releases.sh --keep 1 --public --execute`; alleen `v3.1.79` blijft over als recente source/public release.
- Pi draaide eerder nog `3.1.72`; updater/install via oude versies kon rebooten tijdens monolithische dependency-install.
- Validatie voor `v3.1.79`:
  - `bash -n scripts/install.sh`
  - `python3.11 -m pytest tests/test_installation_contract.py tests/test_updater.py -q` -> 56 passed
  - GitHub Actions publish workflow voor `v3.1.79` -> success
...
- Deploy/updater op Pi gebruikt vanaf `v3.1.79` resumable dependency install markers voor reboot/interruption retries, inclusief losse PySide6-subpackages, en de handmatige installer schakelt eerst de oude updater-timer en client service uit.
