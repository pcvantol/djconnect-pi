...
def test_client_api_info_and_pairing_info(tmp_path: Path) -> None:
    ...
    assert pairing["capabilities"]["ask_dj_free_input_supported"] is False
...
def test_web_portal_media_buttons_and_refresh_are_clickable() -> None:
    ...
    assert "itemHtml(i,'play_context_at')" in html
    assert "itemHtml(i,'start_queue_item')" not in html
