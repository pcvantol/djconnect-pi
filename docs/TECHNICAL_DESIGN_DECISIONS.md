...
| Music DNA and Ontdek stay server-side | `loadMusicDna`, `setMusicDnaEnabled`, `clearMusicDna`, `loadMusicDiscovery`, `refreshMusicDiscovery`, `playMusicDiscoveryItem`, `setMoodValue`, `HAClient.music_dna_*`, `HAClient.music_discovery_*` | Home Assistant is the source of truth for Music DNA and Music Discovery. The Pi renders profile/feed responses, gates Ontdek behind Music DNA consent, hides missing or ineligible blocks, dedupes backend rows by `id`/`uri` for display, shows backend reason text in a full-screen Waarom details view and sends Play Now selections back through `/api/djconnect/v1/music_discovery/play` with `section_id` and `discovery_item_id`; it never calculates recommendations, reasons or Music DNA locally. The local mood selector only stores a 0..100 context value and forwards it to HA. |
