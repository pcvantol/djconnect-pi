...
| Kiosk-first navigation | bottom menu bar, Meer overflow and no visible app close button | Device is intended to be wall-mounted and dedicated; primary navigation keeps Speelt nu, Ask DJ, Track Insight, Ontdek, Music DNA and Meer reachable on the 720x720 display. |
...
| Music DNA and Ontdek stay server-side | `loadMusicDna`, `setMusicDnaEnabled`, `clearMusicDna`, `loadMusicDiscovery`, `refreshMusicDiscovery`, `playMusicDiscoveryItem`, `HAClient.music_dna_*`, `HAClient.music_discovery_*` | Home Assistant is the source of truth for Music DNA and Music Discovery. The Pi renders profile/feed responses, gates Ontdek behind Music DNA consent, hides missing or ineligible blocks, dedupes backend rows by `id`/`uri` for display and sends Play Now selections back through `/api/djconnect/v1/music_discovery/play` with `section_id` and `discovery_item_id`; it never calculates recommendations, reasons or Music DNA locally. |
