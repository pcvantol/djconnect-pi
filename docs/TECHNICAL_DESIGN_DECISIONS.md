...
| Ask DJ stays server-side | `sendAskDjMessage`, `pollAskDjHistory`, `sendAskDjAction`, `HAClient.ask_dj_message`, `HAClient.ask_dj_history`, `HAClient.ask_dj_action`, `askDjPanel` | The Pi sends typed text with `audio_response:auto`, renders only backend-returned message fields and sends backend action payloads back through `/api/djconnect/command`; it does not parse visible text, reuse previous artwork, store memory, reconstruct prompts, infer playback or create local TTS/audio bubbles. Read-only `technical_track_analysis` responses render backend-provided sections, timeline, DJ tips and limitations, show optional `providers[]` as compact diagnostics, and only expose playback controls when explicit `playback_actions[]` are present. |
