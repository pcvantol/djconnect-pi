...
| Ask DJ is readonly with structured actions | `pollAskDjHistory`, `sendAskDjAction`, `HAClient.ask_dj_history`, `HAClient.ask_dj_action` | The Pi has no keyboard, microphone, speaker or TTS path, so it only displays shared Ask DJ history and returns HA-provided action payloads from touch buttons. |
