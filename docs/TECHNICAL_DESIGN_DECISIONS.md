...
| Ask DJ and Track Insight stay server-side | `sendAskDjMessage`, `pollAskDjHistory`, `sendAskDjAction`, `openTrackInsight`, `HAClient.ask_dj_message`, `HAClient.track_insight`, `askDjPanel` | The Pi sends typed text with `audio_response:auto`, renders only backend-returned message fields and sends backend action payloads back through `/api/djconnect/v1/command`; it does not parse visible text, reuse previous artwork, store Music DNA locally, reconstruct prompts, infer playback or create local TTS/audio bubbles. Track Insight uses `/api/djconnect/v1/track_insight` or Ask DJ responses with `intent/action/type/open_screen:"track_insight"`, renders normalized `track_insight` data, treats `visual_profile` as hints and only exposes playback controls when explicit `playback_actions[]` are present. |
| Music DNA and Ontdek stay server-side | `loadMusicDna`, `setMusicDnaEnabled`, `clearMusicDna`, `loadMusicDiscovery`, `refreshMusicDiscovery`, `playMusicDiscoveryItem`, `HAClient.music_dna_*`, `HAClient.music_discovery_*` | Home Assistant is the source of truth for Music DNA and Music Discovery. The Pi renders profile/feed responses, gates Ontdek behind Music DNA consent, hides missing or ineligible blocks and sends Play Now selections back through `/api/djconnect/v1/music_discovery/play`; it never calculates recommendations, reasons or Music DNA locally. |
