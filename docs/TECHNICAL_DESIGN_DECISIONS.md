...
| Ask DJ and Track Insight stay server-side | `pollAskDjHistory`, `sendAskDjAction`, `openTrackInsight`, `HAClient.ask_dj_history`, `HAClient.track_insight`, `askDjPanel`, `trackInsightPanel` | The Pi renders Home Assistant Ask DJ history/status, sends only backend action payloads back through `/api/djconnect/v1/command` and exposes no free prompt input or local history clear; it does not parse visible text, reuse previous artwork, store Music DNA locally, reconstruct prompts, infer playback or create local TTS/audio bubbles. Track Insight uses `/api/djconnect/v1/track_insight` or Ask DJ responses with `intent/action/type/open_screen:"track_insight"`, renders normalized direct/wrapped `track` and `analysis` data, clears stale data on track changes/no-track/rate-limit states, filters BPM/key/model fields and only exposes playback controls when explicit `playback_actions[]` are present. |
