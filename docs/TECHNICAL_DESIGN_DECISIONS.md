...
| Ask DJ text stays backend-owned | `sendAskDjMessage`, `pollAskDjHistory`, `sendAskDjAction`, `HAClient.ask_dj_message`, `HAClient.ask_dj_history`, `HAClient.ask_dj_action` | The Pi can send typed text to Home Assistant and return HA-provided action payloads from touch buttons, but it does not reconstruct intents, memory, follow-ups, voice, speaker output or TTS locally. |
